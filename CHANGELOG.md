...
`s2_msi_raw_generator` runs a Sentinel-2B **L1B** backwards through the exact inverse of the
operational L0→L1B radiometric chain (invert offset, relative-response/PRNU, dark, un-bin, SWIR
re-stage, defective, crosstalk, on-board-eq) to reconstruct **L1A → L0plus (CCSDS-122 ISP) → Synthetic L0**.
MTF-deconvolution is OFF, so PSF and noise are not re-applied. Success = the Synthetic L0 vs the
reference ESA L0 `img` (≤~4 DN on the 10/20 m bands); the L0plus codec round-trip `decode(L0plus)==L1A`
is bit-exact (supporting check).

## [Unreleased]

### Added
- **GIPP JSON parser** — `gipp.py` reads band-organised `aux/gipp-json/{Bxx}/S2B_ADF_*.json` (REQOG,
  RDEPI, BLIND, RPARA, RCRCO) in addition to inline XML test fixtures.
- **Centralised environment module** — `s2_msi_raw_generator.env` loads `.env` and exposes
  `OUTPUT_DIR`, `S2_GIPP_DIR`, `S2_AUX_DIR`, `S2_L1B_INPUT`, `S2_L0_INPUT` as the single path source.
- **`.env.example`** — template for required path variables and optional `S2_*` tuning knobs.

### Changed
- **Environment migration** — all `S2_E2ES_*` and `S2_DATA_STORE` names replaced by `S2_*` and
  `OUTPUT_DIR`; pipeline reads equalization from `S2_GIPP_DIR` (gipp-json) only — no `eqog_adf`
  override on the production path.
- **Removed fallbacks** — `LOCAL_ENV_DEFAULTS`, `~/data-store`, VM-specific paths, and
  `PDI_MSI_S2_L1A.zarr` production default are gone; missing `.env` variables exit with a clear error.
...
- **Data-store inventory and consistency report** — new `s2_msi_raw_generator.inventory`
  ...
  generated products, public Synthetic L0/Synthetic L0P references, cal-DB entries, reports, staging zips and broken
...
- **Public-L0 same-scene bridge** — new `s2_msi_raw_generator.import_l0` module and
  ...
  the PDI-style L1A layout consumed by the pipeline, preserving STAC orbit/platform identity
  and recording `import_provenance` plus per-band CRCs for A0 bit-exact copy checks.
- **`notebooks/compare_synthetic_l0.ipynb`** — the reverse chain's headline validation: the
  reverse-chain Synthetic L0 next to the reference ESA L0 `img` (`inputs/public-data/level-0/
...
- **`notebooks/pipeline_walkthrough.ipynb`** — the reverse-chain walkthrough with an image
  per rung: S2 L1B window → invert offset → relative-response/PRNU → dark → un-bin →
  SWIR re-stage → defective → crosstalk → on-board-eq → L1A/L0plus/Synthetic L0, with
...
- **Calibration mode (the `calibration` positional, REQ-FUNC-048)** — synthesizes the calibration
  ...
  **downlink Synthetic L0 product**: CCSDS-122 compressed ISPs under the PSFD §3 calibration type
...
- **The `--synthetic` flat-field demo chain** (`build-l0-synth`/`l0-to-l1b` phases,
  ...
  (`flatfield.zarr`, `dark.zarr:/frame`) are superseded by the calibration Synthetic L0 products
  (DJF DEC-13).
...
- **Process-pool and zip helpers** — the spawn-context `ProcessPoolExecutor` fan-out (canonical Synthetic L0
...
- **Orbit identity threading** — pipeline preflight now records platform/orbit acquisition
  ...
  0 (`A000000`) instead of mixing the A045 file-name default with an unrelated synthesised orbit.
- **Multi-core data generation (`S2_JOBS`, default: all cores)** — the per-band CPU
...
- **Mode-only CLI, environment-driven configuration** — `run_pipeline.py` now takes just
  the mode (`nominal` default | `calibration`); the store root comes from `$OUTPUT_DIR`
  (default `~/data-store`) and every tuning knob is an `S2_*` variable
...
- **Shared data-store** — products and inputs move to the `ipf/data-store` project's
  ...
  `publish-e2e-s2-l1b` CI job becomes `publish-datastore` (parameterised PKG/VER/LAYER).
...
- **Single pipeline driver** — the ten `scripts/` entry points are consolidated into one
  phase-structured `scripts/run_pipeline.py` (the reverse-chain phases from the former
  `run_pipeline.py`; on-demand phases `build-caldb` / `derive-adf` / `figures` absorb
...
- **Docs landing page** — the documentation front page (`docs/index.md`) now leads with the
  reverse chain: a S2B L1B run backwards through the exact inverse of the
  operational L0→L1B radiometric chain to reconstruct L1A/L0plus/Synthetic L0, validated against the
  reference ESA L0 `img` (≤~4 DN on the 10/20 m bands). The separate `showcase` page was removed.
  REQ-FUNC-093 is rescoped to the S2 L1B reconstruction/validation driver.

## [0.3.0] — 2026-07-02

S2 L1B E2E release: delivers the machinery the reverse chain reuses to run an
Sentinel-2B L1B backwards to L1A → L0plus → Synthetic L0 — the **CCSDS-122 lossless codec**, real
...
(`e2e-s2-l1b/0.3.0` generic packages) + validation report (`docs/vv/s2_l1b_e2e.md`).

### Added
- **S2 L1B reverse chain driver + PSFD naming + bucket fetch** — `scripts/run_pipeline.py`
  (phase-structured, idempotent: fetch-l1a/fetch-l0/preflight/package/ground-decode/l0-decode/
  validate/scan-l0/quicklook/report; REQ-FUNC-093): reconstructs the public bucket L1A into the
  compressed-ISP canonical Synthetic L0 + open-container form under **EOPF PSFD §3 names** (new
  ...
  **L0plus codec round-trip** (`decode(L0plus)==L1A`, bit-exact) via msi-processor `l0_decode`,
  accounts for line-loss, EOQC, and a **structural scan of a ESA PSD L0 SAFE** (packet-tiling
  criterion on its ISP `.bin` files). New stdlib-only `s3fetch.py` (anonymous S3 listing +
  verified parallel GET). New manual CI job **`e2e-s2-l1b`** (windowed, artifacts:
  report+quicklooks). Docs: ICD-IF-NAME, SRS REQ-FUNC-091/092/093, CIDL rows,
  `docs/vv/s2_l1b_e2e.md`, README/DPM diagrams updated to the reverse-chain flow.
- **Compressed ISP payloads + ground decode (downlink shape)** — the canonical Synthetic L0's
  `with_isp` branch now CCSDS-122-compresses each band and carries it as **CCSDS space
  ...
  `write_l0_product(..., store_decoded=False)` stores **ISPs only**, mirroring the ESA S2 Synthetic L0.
  `io.read_l1a_raw` gains a `dtype` kwarg (uint16 reads without the float64 spike). (ICD-IF-Synthetic L0
  updated; REQ-FUNC-092)

### Changed
- **Removed the per-line `isp_header` array** from the canonical Synthetic L0 (superseded by the packet
...
- **CCSDS 122.0-B lossless image compression** (`s2_msi_raw_generator/ccsds122.py`, pure numpy) —
  ...
  decoder; `compress_frame`∘`decompress_frame` is bit-exact (14 unit tests + env-gated S2 L1B
  window). Full 21384×2592 band ≈ 19 s each way. Groundwork for compressed ISP payloads
  (REQ-FUNC-092; ATBD §5.S15 rewritten to the two-step onboard chain).
...
- **zarr v2/v3 write compatibility** (`s2_msi_raw_generator/_zarrio.py`) — the L0 + cal-DB writers now run under
  **both** zarr 3 (local/CI) and **zarr 2.18** (the `eopf==2.8.1` SDE env; eopf pins zarr <3). Products stay in
  the zarr **v2** on-disk format either way, so the reverse-chain writers run in a single venv alongside
...
- **Satellite Ancillary Data** (`s2_msi_raw_generator/sad.py`) — replaces the placeholder all-zero SAD
  payload with telemetry: a synthesised Sentinel-2 sun-synchronous orbit (ECEF position/velocity), a
  ...
  plus a CCSDS outer-framing decoder (`scan_ccsds_packets`/`decode_sadata_framing`) for SADATA/HKTM
...
- **Quality-flag taxonomy** (`s2_msi_raw_generator/quality.py`) — L0 quality expressed as msi-processor
  ...
  interop); the canonical Synthetic L0 mask is now the Sentinel-2 `MSK_QUALIT` 8-bit-plane layout
...
- **EOQC quality report** (`s2_msi_raw_generator/quality_report.py`) — EOPF EOQC-style per-product report
  ...
  Time_Correlation, orbit bounds, structure), embedded in the Synthetic L0 `quality` group and writable as standalone
  JSON. ECSS-Q-ST-20C. (REQ-FUNC-041)
- **GPS/OBT line datation** (`s2_msi_raw_generator/datation.py`) — `Datation` (ADF_DATAT model) stamps each ISP
  line with a GPS/OBT time from an acquisition epoch (was `t0=0`); `isp.parse_cuc_time`; per-band
...
- **L0 STAC geometry & orbit metadata** — `build_root_metadata` now writes the footprint (`bbox` + closed
  ...
  `processing:*`, `eopf:datastrip_id`, and a **acquisition datetime span** (start/end from datation) with S2A
...
- **Operational GIPP reader** (`s2_msi_raw_generator/gipp.py`) — original `xml.etree` parser of the S2A
...
- **ATBD radiometric inversion core** (`s2_msi_raw_generator/forward_radiometric_atbd.py`) — the exact
  **inverse** radiometric transform from the public L1 ATBD, driven by the GIPP: the reverse chain's
...
- Full reverse radiometric-inversion reverse chain (invert offset, relative-response/PRNU, dark, un-bin,
...
- Synthetic L0 RAW EOProduct assembly (156-array Zarr + STAC/sensor-config).
- SRF spectral characterisation (sensor model); S2 PSF matrices (SentiWiki) retained only as an
  archived sensor reference (PSF/MTF is not re-applied in the reverse chain).
