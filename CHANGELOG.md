...
- **Calibration sub-set** (`s2_e2es/calibration.py`) — the S2 two-reference radiometric calibration:
  ...
  truth — the inverse-crime cure. Verified on the real GIPP (dark <0.05 DN, `g` corr >0.99, `A≈cal_gain`).
- **Real operational GIPP reader** (`s2_e2es/gipp.py`) — original `xml.etree` parser of the real S2A
  GIPP: R2EQOG (per-pixel dark `COEFF_D` + cubic/bilinear relative-response gains), R2DEPI, BLINDP,
  R2PARA, R2CRCO. `BandADF.from_gipp()` builds real per-pixel dark + PRNU ADFs.
- **Original ATBD forward + round-trip V&V** (`s2_e2es/forward_radiometric_atbd.py`) — forward
  ...
  validates forward∘reverse to ~1e-14 RMSE on a **real L1A** with the real GIPP.
...
- Per-band noise model uses the real product `noise_model` (α, β; S2-RUT `σ=√(α²+β·DN)`).
- Dark/PRNU now real per-pixel from the GIPP (was DQR-summary / seeded).
...
- Real ESA PSF matrices (SentiWiki) + real SRF spectral characterisation.
- Sensor model with real per-band gains/TDI/line-period from EOPF products.
