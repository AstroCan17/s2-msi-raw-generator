...
Runs a **S2B L1B** backwards through the **exact inverse of the operational
L0→L1B radiometric chain** to reconstruct the full EOPF product **L1A → L0plus
(CCSDS-122 ISP) → Synthetic L0** (focal-plane DN `img`, 12 detectors × 13 bands). The Synthetic L0
is validated directly against the **reference ESA L0 `img`**, agreeing to **≤ ~4 DN on the ten
10 m + 20 m bands** (the three native-60 m bands are un-bin-limited)
([see below](#reverse-l1b--l1a--l0plus--l0--s2-l1b-validation)).
...
The generator runs S2B **L1B** backwards through the inverse radiometric chain and
materialises each EOPF product level — **L1A → L0plus → Synthetic L0 `img`** — then validates the
Synthetic L0 against the **reference ESA L0 `img`**. It *also* derives a **calibration
...
flowchart LR
    ...
    L0[("Synthetic L0 — decoded img<br/>(PSFD S02MSIL0__…)")]
    REFL0[("reference ESA L0 img<br/>(2024-04-08 S2B)")]
    ...
    VAL["validate-reverse:<br/>Synthetic L0 vs reference ESA L0 img (≤ ~4 DN)"]
    L1[("L1B<br/>product")]
    IN --> REV
    ADFsrc --> REV
    ADFsrc --> CAL
...
S2 L1B runs add `inputs/`, `l1a_prime/`, `report/`) with **EOPF PSFD §3** file names
(ICD-IF-NAME). The full reverse chain (S2 L1B → L1A → L0plus → Synthetic L0 `img` → validate vs reference ESA L0)
is driven by `scripts/run_pipeline.py` (see `docs/vv/s2_l1b_e2e.md`). As a supporting check, the
...
## Reverse L1B → L1A → L0plus → Synthetic L0 — S2 L1B validation

The generator's headline capability is the **exact reverse of the operational L0→L1B radiometric
chain**, materialised as the **full EOPF EOPF product chain** (the canonical forward chain
`Synthetic L0 → L0plus → L1A → L1B` run backwards): given S2B **L1B** (digital counts),
...
- **L1A** (`reverse-l1b` phase → `import_l0.write_l1a_product`): decompressed raw counts,
  `measurements/DD{dd}/{BAND}/l1a_raw_image` — the same layout the EOPF L1A/Synthetic L0-`img` carries.
...
- **L0** (`package-l0` → `l0product.write_l0_decoded_product`, `S02MSIL0_`): the L0plus ISP decoded back
  to `measurements/d{DD}/b{BB}/img` + decode-quality attrs — **format-identical to the reference S2B
  `S02MSIL0__…` product** (verified against the 2024-04-08 TC7D granule), for a direct array comparison.

`validate-reverse` compares the Synthetic L1A against the **reference S2B L0 `img`** directly (no decoding —
the archived EOPF L0 already stores decompressed `img`), framing-aligned to the ADF_PRDLO co-registration
crop. Validated on the **2024-04-08 S2B PPB** Synthetic L0/L1B pair (detector d05):
...
**Synthetic L1A vs reference ESA L0 `img` — all 13 bands, detector d05, framing-aligned (drift 0):**
...
![Full-chain reverse — Synthetic L0 vs reference ESA L0, all 13 bands (Synthetic L0 | ESA L0 | diff); the diff panels are flat to a few DN for the 10/20 m bands, textured for the three 60 m bands](docs/_static/showcase/reverse_l1b_allbands.png)

> *The "reference S2B L0" panels above contain **modified Copernicus Sentinel data 2024** (Sentinel-2B,
> 2024-04-08 datatake), used here for validation and shown as low-resolution demo previews only — no raw
> product data is redistributed in this repository. Input products (Synthetic L0/L1B) and operational GIPP/ADF are
...
`S2_PHASES=reverse-l1b,package-l0,validate-reverse S2_L1B_INPUT=<L1B.zarr> S2_L0_INPUT=<Synthetic L0.zarr> python scripts/run_pipeline.py`
(auto-finds RSWIR/REOB2/RCRCO under `$S2_AUX_DIR/adf-eopf`; `S2_REVERSE_FULL=0` →
radiometric-only; `S2_JOBS=1` on NFS-backed stores). `validate-reverse` needs `$S2_L0_INPUT`
and ADF_PRDLO framing offsets (`$S2_FRAMING` or auto-found under `{S2_AUX_DIR}/framing/`,
...
| `s2_msi_raw_generator/adf.py` | ADFs — **ESA** PSF matrices (`data/psf/`) + SRF spectral; PRNU/dark from the operational GIPP (`BandADF.from_gipp`) |
...
| `s2_msi_raw_generator/forward_radiometric_atbd.py` | **`reverse_l1b_to_l0`** — the reverse chain core: the full S2-S2 L1B→Synthetic L0 inverse (offset/PRNU/dark/un-bin **+ S8 SWIR re-stage + S10 defective + S12 on-board eq**), with `np.clip`/`np.rint`→`uint16` quantization to the 12-bit L0 data format |
...
| `s2_msi_raw_generator/reverse.py` | Legacy forward-impress engine (steps **S1–S14**, incl. the now-OFF S6 PSF re-blur and S13 noise) + `reverse_mvp` — imported at module load, **not part of the reverse chain** |
| `s2_msi_raw_generator/isp.py` | **S15** — CCSDS ISP packet generation + SAD telemetry |
| `s2_msi_raw_generator/io.py` | EOPF L1A/L1B Zarr reader (`zarr`) |
| `s2_msi_raw_generator/import_l0.py` | Public Synthetic L0 → PDI-style L1A bridge (`import-l0`) **+ `write_l1a_product`** — the reverse chain's materialised L1A writer (`reverse-l1b` phase) |
| `s2_msi_raw_generator/inventory.py` | Metadata-only data-store inventory + consistency report (`inventory` phase) |
| `s2_msi_raw_generator/l0product.py` | Synthetic L0 RAW EOProduct assembly (156-array Zarr + STAC/sensor-config + ISP) |
...
| DRD | File | Content |
|---|---|---|
| ATBD | `docs/atbd/atbd.md` | Reverse-chain algorithm basis — the inverse of the L0→L1B chain + Annex A datasheet (issued v1.0) |
...
| ICD | `docs/icd.md` | Interfaces — L1A/L1B + GIPP inputs, the Synthetic L0 RAW output (ICD-IF-Synthetic L0) |
| DPM | `docs/dpm/` | Data processing model — the reverse chain blocks + parameter/data list |
| V&V | `docs/vv/` | Verification & validation plan + report (201 tests at v0.3.0; Synthetic L0 vs the reference ESA L0 `img` to ≤ ~4 DN on the 10/20 m bands: vv/s2_l1b_e2e) |
...
**CHANGELOG** — `CHANGELOG.md`. **License** — `LICENSE` (Apache-2.0). All instrument data is ESA-sourced
ESA-sourced (official PSF, SRF, product noise model, operational GIPP) — nothing fitted;
...
the store root is `$OUTPUT_DIR` (set in `.env`) and tuning knobs use `S2_*` environment variables:

```bash
cp .env.example .env   # set S2_L1B_INPUT, S2_L0_INPUT, S2_GIPP_DIR, S2_AUX_DIR, OUTPUT_DIR
# nominal mode (default): S2B L1B → reverse chain → L1A / L0plus / Synthetic L0 → <store>/l0/,
# then validate the Synthetic L0 against the reference ESA L0 img
...
# downlink Synthetic L0 products (S02MSIDCA / S02MSISCA, compressed ISPs) + Option-Y cal-DB,
...
S2_PHASES=publish-store S2_PUBLISH_VERSION=<X.Y.Z> python scripts/run_pipeline.py

# on-demand phases
S2_PHASES=inventory python scripts/run_pipeline.py                        # INVENTORY.md + report/inventory.json
S2_PHASES=import-l0,preflight,package,ground-decode,l0-decode,validate,report \
  S2_L0_INPUT=<S02MSIL0__.zarr.zip> python scripts/run_pipeline.py        # same-scene bridge
S2_PHASES=build-caldb python scripts/run_pipeline.py                        # Option-Y cal-DB ADFs
S2_PHASES=derive-adf S2_L1A_INPUT=<L1A.zarr> python scripts/run_pipeline.py  # operational PRNU/dark → npz
python scripts/reverse_compare_figure.py                                         # reverse-chain all-bands figure
```

The GIPP folder holds the `S2A_OPER_GIP_*.xml` files (R2EQOG ×13, R2DEPI, BLINDP, R2PARA,
R2CRCO); the L1A is EOPF L1A Zarr (`measurements/DDnn/Bxx/l1a_raw_image`). Products and inputs
live in the shared `ipf/data-store` registry —
pull a working copy with `S2_PHASES=fetch-store`. S2 L1B tests run
when `S2_GIPP_DIR` / `S2_L1A_INPUT` are set. The full variable reference is in
...
**Complete — full reverse chain (S2B L1B → L1A → L0plus (CCSDS-122 compressed ISPs) → Synthetic L0), the Synthetic L0 validated to ≤ ~4 DN vs the reference ESA L0 `img` on the ten 10/20 m bands; 201 tests, CI green.**

| Increment | Content |
|---|---|
| 0 | Scaffold, CI, ATBD + Annex A datasheet |
| 1 | Radiometric core + sensor model + S2 PSF / SRF ADFs |
| 2 | Synthetic L0 RAW EOProduct assembly (156-array Zarr) |
...
| 6 | Operational **GIPP** → per-pixel dark + relative response (`gipp.py`, `from_gipp`) |
| 7 | Reverse-chain inversion of the L0→L1B chain (`reverse_l1b_to_l0`) |
| 8 | S2 **calibration sub-set** — CSM diffuser + dark → derived coeffs (inverse-crime cure) |
| 9 | **Synthetic L0 completion → Synthetic L0→L1B E2E** — ESUN spectral ADF, GPS/OBT datation, STAC geometry/orbit + orbit-ephemeris, SAD (AOCS/orbit/thermal), QAFlag/MSK_QUALIT quality + EOQC report, open-container handoff to `msi-processor` |

**All radiometric ADFs are ESA-sourced** (official ESA PSF, SRF, product noise model, operational GIPP) —
nothing fitted. Runs end-to-end on S2 L1A/L1B with `numpy` + `zarr` only.
