...
|---|---|
| `sensor.py` | Sentinel-2 sensor model — real harvested constants: bands, GSD, physical gains, Lref/SNR, integration time, TDI/SWIR sets, per-unit SRF (centre/bandwidth/equivalent wavelength), noise model α/β, dark pedestal, EQ-gain stability, quantization. `Band` dataclass (`.dn_ref`, `.cal_gain`, `.dark_dsnu`). |
| `gipp.py` | Original parser of the real operational S2A GIPP XML → per-pixel arrays: R2EQOG (dark `COEFF_D` + cubic/bilinear relative-response gains), R2DEPI (defective), BLINDP (blind), R2PARA (offsets/flags), R2CRCO (crosstalk). |
| `adf.py` | Per-band ADF assembly: real ESA PSF matrices (`data/psf/`), real noise coefficients; builds `BandADF` `from_gipp` / `from_product` / `synthesize`. |
...
| `io.py` | Lightweight `zarr` reader for real EOPF L1A/L1B products (no full EOPF dependency). |
...
```
L1B radiance
  → S1 radiance→DN (DN = A·L)        → S3 undo framing → S4 undo offset (−100)
  → S5 undo 60 m binning             → S6 PSF re-blur (real ESA matrices; B10 = identity)
  → S7 impress relative response      → S8 SWIR re-stagger → S9 crosstalk → S10 blind/defective
  → S11 re-apply dark (+D)            → S12 reverse onboard equalization
  → S13 add noise σ=√(α²+β·DN)        → S14 quantize 12-bit
  → S15 format L0 RAW (156 frames + ISP + STAC)
```
...
are flagged in the quality masks; the dark pedestal and per-pixel coefficients come from the real GIPP
when supplied, else fall back to the published DQR/datasheet values.

## Interfaces context
Inputs: EOPF L1A/L1B Zarr products and the real operational GIPP XML. Output: the L0 RAW EOProduct Zarr
